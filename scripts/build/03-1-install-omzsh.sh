...
/bin/echo -e "\e[38;5;39m# Installing Oh-My-ZSH...                     #\e[0m"
/bin/echo -e "\e[38;5;39m#=============================================#\e[0m"
echo
### Oh-My-ZSH
# Get some fonts:
if [ ! -d "$HOME/MesloLGSNF" ]; then
...
sudo mkdir -p '/usr/share/fonts/truetype/MesloLGSNF'
sudo cp -f ~/MesloLGSNF/* '/usr/share/fonts/truetype/MesloLGSNF'
rm -Rf ~/MesloLGSNF

# Set fonts for VS Code and turn off minimap
...
# Note - zsh should be installed first!
bash -c "$(curl -fsSL https://raw.github.com/ohmyzsh/ohmyzsh/master/tools/install.sh)" "" --unattended
# Cleanup powerlevel10k just in case:
# mkdir -p ~/.oh-my-zsh/custom/themes/powerlevel10k
rm -Rf ~/.oh-my-zsh/custom/themes/powerlevel10k

git clone --depth=1 https://github.com/romkatv/powerlevel10k.git ~/.oh-my-zsh/custom/themes/powerlevel10k

# Set plugins and theme preference:
sed -i 's/plugins=(git)/plugins=(git zsh-autosuggestions zsh-syntax-highlighting autojump fzf)/g' ~/.zshrc
...
echo "autoload -U compinit && compinit -u && autoload -U bashcompinit && bashcompinit"  | sudo tee -a /etc/zsh/zshrc
...
## Get the Fuzzy Finder
git clone --depth 1 https://github.com/junegunn/fzf.git ~/.fzf
# Warning: The following has some prompts when no switches added
sudo -E ~/.fzf/install --all
echo "export FZF_BASE=~/.fzf" | sudo tee -a /etc/zsh/zshenv
echo "[ -f ~/.fzf.zsh ] && source ~/.fzf.zsh" | sudo tee -a /etc/zsh/zshrc
echo "[ -f ~/.fzf.bash ] && source ~/.fzf.bash" | sudo tee -a /etc/bashrc
sudo chown -R "${USER}"."${USER}" ~/.fz*
sed -i 's|/home/.*/.fzf|$HOME/.fzf|g' ~/.fzf.zsh
sed -i 's|/home/.*/.fzf|$HOME/.fzf|g' ~/.fzf.bash

if [ -f ~/.zshrc ]; then
  # Set plugins and theme preference: # Ucomment to allow ZSH auto update without prompt:
  sed -i 's/plugins=(git)/plugins=(git zsh-autosuggestions zsh-syntax-highlighting docker docker-compose fzf)/g' ~/.zshrc
  sed -i 's/^ZSH_THEME.*/ZSH_THEME=\"powerlevel10k\/powerlevel10k\"/' ~/.zshrc
  sed -i '/^# DISABLE_UPDATE_PROMPT="true"/s/^# //' ~/.zshrc
fi

mv ~/.zshrc ~/.zshrc.orig
#########################################################
# This first format would replace variables in the script as it is saved, making it user-specific
## cat <<EOF > ~/.zshrc 
# This second format leaves the variables as is so that the script can be used by anyone!
cat > ~/.zshrc << "EOF"
...
plugins=(git zsh-autosuggestions zsh-syntax-highlighting docker docker-compose autojump fzf)
...
# Now pre-create a .p10k.zsh config based on my preferred selections:
cat > ~/.p10k.zsh << "EOF"
# Generated by Powerlevel10k configuration wizard on 2022-03-22 at 20:50 UTC.
...
# dark-ornaments, compact, many icons, fluent, transient_prompt, instant_prompt=quiet.
...
() {
  ...
  typeset -g POWERLEVEL9K_KUBECONTEXT_SHOW_ON_COMMAND='kubectl|kubecolor|helm|kubens|kubectx|oc|istioctl|kogito|k9s|helmfile|flux|fluxctl|stern|kapp'
...
EOF
# Pre-initialize the powerlevel10k gitstatusd as per https://github.com/romkatv/powerlevel10k/issues/747
# echo exit | script -qec zsh /dev/null >/dev/null 
# The option above doesn't work here... trying this one below:
~/.oh-my-zsh/custom/themes/powerlevel10k/gitstatus/install
#########################################################
echo "03-install-omzsh.sh executed" >> "$HOME"/.packer.txt