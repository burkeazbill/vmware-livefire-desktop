/*
    DESCRIPTION: Ubuntu Server 22.04 LTS variables using the Packer Builder for VMware vSphere (vsphere-iso).
*/

// vCenter Connection Settings
variable "vsphere_endpoint" {
  type        = string
  description = "The fully qualified domain name or IP address of the vCenter Server instance. (e.g. 'vcenter.example.com')"
}
...
variable "vsphere_datacenter" {
  type        = string
  description = "The name of the target vSphere datacenter. (e.g. 'datacenter-01')"
}

variable "vsphere_cluster" {
  type        = string
  description = "The name of the target vSphere cluster. (e.g. 'cluster-01')"
}
...
variable "vsphere_datastore" {
  type        = string
  description = "The name of the target vSphere datastore. (e.g. 'vsanDatastore')"
}

variable "vsphere_switch" {
  type        = string
  description = "The name of the vSphere switch the network segment is attached to"
}

variable "vsphere_network" {
  type        = string
  description = "The name of the target vSphere network segment. (e.g. 'dhcp-net')"
}

variable "vsphere_folder" {
  type        = string
  description = "The name of the target vSphere cluster. (e.g. 'Templates')"
}

// Virtual Machine Settings
variable "vm_name" {
  type    = string
  description = "The base name of the VM to be created"
  default = "ubuntu"
}
...
variable "vm_cpu_cores" {
  type        = number
  description = "The number of virtual CPUs cores per socket. (e.g. '2')"
}
...
variable "vm_video_ram" {
  type    = number
  description = "The size of video memory in KB. (e.g. '65536')"
  default = 65536
}
...
variable "vm_nested_hardware_virtualization" {
  type        = bool
...
variable "common_create_snapshot" {
  type        = bool
  description = "Create a snapshot of the VM when complete."
  default     = true
}
variable "common_snapshot_name" {
  type        = string
  description = "Name of snapshot, if undefined: Created by Packer"
...
// Output Settings
// Docs: https://www.packer.io/plugins/builders/vsphere/vsphere-iso#export-configuration

variable "common_export_ovf" {
  type        = string
  description = "Flag to determine whether or not to export VM to OVF when done"
  default     = "no"
}

variable "common_output_path" {
  type        = string
  description = "Output folder for generated artifacts such as manifests and exported vms"
  default     = ""
}

variable "common_output_force_overwrite" {
  type        = bool
  description = "Overwrite ovf if it exists"
  default     = false
} 
...
variable "iso_url" {
  type        = string
  description = "The URL to retrieve the ISO image from"
}
...
variable "build_username" {
  type        = string
  description = "The username to login to the guest operating system. (e.g. 'vmadmin')"
  sensitive   = true
}
...
variable "common_private_dns_ip" {
  type        = string
  description = "Your internal private DNS Server IP. This will be configured in the /etc/resolv.conf"
}

variable "common_private_ntp_ip" {
  type        = string
  description = "Your internal private NTP Server IP or FQDN. This will be configured in /etc/systemd/timesyncd.conf"
}

variable "common_internal_domain_name" {
  type        = string
  description = "Your internal network domain name. This will be configured as the search domain in /etc/resolv.conf"
}
